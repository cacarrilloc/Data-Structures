 ...
 ** Date:   02/19/2016                                      *
 ** Description: This program shows a heap implementation   *
 *  using a dynamic array, a stack, and a bag.              *
...
void addHeap(DynArr *heap, TYPE val, comparator  compare){
    ...
    /* Set variables */
    int parent;
    int pos = sizeDynArr(heap); /* Start at size index */
    ...
    while(pos != 0){
        ...
        /* Parent of node is at floor */
        parent = (pos - 1) / 2;
        
        if(compare(getDynArr(heap, parent), getDynArr(heap, pos)) == 1){
            /* Swap nodes if child is greater than parent */
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare){
    ...
    if (rightChild <= max){ /* we have two children */
        ...
        smallerChild = _smallerIndexHeap(heap, leftChild, rightChild, compare);
        
        if (compare(getDynArr(heap, smallerChild), getDynArr(heap, pos)) == -1){
            /* Swap with smallest child */
            swapDynArr(heap, smallerChild, pos);
            /* Adjust heap */
            _adjustHeap(heap, max, smallerChild, compare);}}
...
void removeMinHeap(DynArr *heap, comparator compare) {
    ...
    /* Find the last element in the tree/array */
...
void _buildHeap(DynArr *heap, comparator compare){
    ...
    /* First possible unbalance node */
    int nodeIndex = (sizeDynArr(heap) / 2) - 1;
    ...
    while(nodeIndex >= 0){
        /* Adjust heap */
        _adjustHeap(heap, sizeDynArr(heap) - 1, nodeIndex, compare);
        nodeIndex--;}
}
/*
...
void sortHeap(DynArr *heap, comparator compare){
    ...
    /* get the highest index in the heap */
    int lastIndex = sizeDynArr(heap) - 1;
    
    /* Check array is not empty */
    assert(sizeDynArr(heap) > 0);
    
    /* Make sure the tree is actually a heap */
    ...
    while(lastIndex >= 0){
        ...
        /* Decrease index of the last element */
        lastIndex--;
